...
def redrawWindow(player, player2, flicker, counter, power, s):
    screen.fill('black')
...
def main():
    ...
    while run:
        clock.tick(fps)
        p2Pos = read_pos(n.send(make_pos((p.x, p.y, p.direction, p.dot, p.power, reset, endgame)))) #p.dot was me trying to fix my eating problem
        p2.x = p2Pos[0]
        p2.y = p2Pos[1]
        ...
        if power:
            if not eaten_ghosts[0]:
                ...
                if pygame.Rect.colliderect(p2.rect,p.rect) and not eaten_ghosts[0]:
                    ...
                    eaten_ghosts[0] = True
                    p2.images = ghost_images
                    p2Pos = read_pos(n.send(make_pos((p.x, p.y, p.direction, p.dot, p.power, reset, endgame))))
                p.dot = 0
        ...
        redrawWindow(p, p2, flicker, counter, power, shadow_objects)
